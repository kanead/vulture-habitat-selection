...
north_data <- filter(north_data,species=="wb")
north_data
length(north_data$time)
...
# try the amt package 
trk <- mk_track(north_data, .x=long, .y=lat, .t=time, id = id, 
...
#' if some data points look a little off
#' we can identify them to investiage further and remove them
#' if needs be
# filter(trk,id=="5201" & nsd_ > 1)
...
#' Spot will have his resolution coarsened for this
trk<- filter(trk,id=="Spot_33525"|
               id=="5201"|
               id=="5208"|
               id=="5206"|
               id=="Santa")
...
head(ssf.df.out)
write.csv(ssf.df.out, file="movebank/NorthSSFannotate.csv", row.names = FALSE)
