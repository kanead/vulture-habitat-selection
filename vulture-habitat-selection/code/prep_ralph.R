...
ralph_data <- filter(ralph_data,species=="wb")
ralph_data
...
# try the amt package 
trk <- mk_track(ralph_data, .x=long, .y=lat, .t=time, id = id, 
...
head(ssf.df.out)
write.csv(ssf.df.out, file="movebank/RalphSSFannotate.csv", row.names = FALSE)
