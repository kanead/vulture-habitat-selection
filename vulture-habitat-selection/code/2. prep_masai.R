...
masai_data <- filter(masai_data,species=="wb")
masai_data
...
# try the amt package 
trk <- mk_track(masai_data, .x=long, .y=lat, .t=time, id = id, 
...
head(ssf.df.out)
write.csv(ssf.df.out, file="movebank/MasaiSSFannotate.csv", row.names = FALSE)
